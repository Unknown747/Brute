...
const (
        ...
        DEFAULT_RPC = "http://202.61.239.89:8545"
        // ===================================================
)
...
func parseConfig() *config {
        ...
        flag.StringVar(&cfg.rpc, "rpc", "", "URL RPC lengkap (contoh: http://ip:8545) — override -server dan -port")
        flag.StringVar(&cfg.server, "server", "", "RPC server (host saja, pakai -rpc untuk URL lengkap)")
...
// primaryURL menentukan URL RPC utama dari flag atau DEFAULT_RPC
...
const (
        batchMin      = 5
        batchMax      = 100
        batchAdjEvery = 10 // setiap N batch, evaluasi ulang ukuran
)
...
// ============================================================
// BALANCE CHECK — batch RPC, adaptive, retry, graceful shutdown
// ============================================================
...
func main() {
        ...
        pool, err := newRPCPool(primary, backupURLs)
        if err != nil {
                log.Fatalf("%v\n", err)
        }
        ...
        if cfg.mode1 {
                ...
                // Jalankan checker goroutines
                for t := 0; t < cfg.threads; t++ {
                        wg.Add(1)
                        go checkBalance(ctx, chData, pool, cfg.timeout, ab)
                }

                // Jalankan multiple producer goroutines — masing-masing punya rand sendiri
                ...
                // Tunggu semua producer selesai setelah ctx cancel
                prodWg.Wait()
