...
var (
        ...
        // FIX #1: channel tunggal untuk tulis last_key.txt — tidak ada goroutine bocor
...
// startKeyWriter menjalankan satu goroutine permanen untuk menulis last_key.txt
// FIX #1: menggantikan pola "go writeLastKey(pk)" yang bisa bocor goroutine
func startKeyWriter() {
        go func() {
...
// sendLastKey mengirim key terbaru ke writer tanpa memblok
// Jika writer sedang sibuk, key lama dibuang dan key baru dikirim
func sendLastKey(key string) {
        select {
        case lastKeyCh <- key:
        default:
                // writer sedang sibuk — ganti isi channel dengan key terbaru
                select {
                case <-lastKeyCh:
                default:
                }
